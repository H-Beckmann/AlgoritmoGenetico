...
#             pai2 = np.random.randint(0, TAM_POP)
    ...
    def cruzamento_composto(self, q):
        pai1 = self.pop[q[0]]
        pai2 = self.pop[q[1]]
        filho1 = np.zeros(self.TAM_GENE, dtype = np.int)
        filho2 = np.zeros(self.TAM_GENE, dtype = np.int)

        primeiroCorte = int(self.TAM_GENE/3)
        segundoCorte = primeiroCorte*2

        for i in range(self.TAM_GENE):
            if i < primeiroCorte or i >segundoCorte:
                filho1[i] = pai2[i]
                filho2[i] = pai1[i]
            else:
                filho1[i] = pai1[i]
    ...
    def cruzamento_mascara(self, q):
        ...
        mascara = np.random.randint(0, 2, self.TAM_GENE)
